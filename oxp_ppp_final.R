...
#mki67 <- dat1["MKI67",]
#pid <- colnames(mki67)
#ki67 <- t(apply(mki67, 1, scale))
#olnames(mki67) <- pid
#mki67 <- t(mki67)
#mki67_median <- cbind(mki67, c3_median, c5_median)
#mki67_median <- na.omit(mki67_median)
#quantile(mki67_median$c3_median)
#ki67_c3median <- subset(mki67_median, subset = (mki67_median$c3_median < 1 & mki67_median$c3_median > -1))
#mki67_c3median <- subset(mki67_c3median, subset = (mki67_c3median$MKI67 < 2 & mki67_c3median$MKI67 > -2))
#ibrary(ggplot2)
#library(ggExtra)
#  geom_point(inherit.aes = TRUE,color = "grey40", size = 1.5) +
#  geom_smooth(method = "lm", color = "blue", fill = "lightblue") +
...
#  fill = c("#21897e")  # top, right
#)
#fit <- lm(MKI67 ~ c3_median, data = mki67_c3median)
#summary(fit)
#quantile(mki67_median$c5_median)
#mki67_c5median <- subset(mki67_median, subset = (mki67_median$c5_median < 1 & mki67_median$c5_median > -1))
#mki67_c5median <- subset(mki67_c5median, subset = (mki67_c5median$MKI67 < 2 & mki67_median$MKI67 > -2))
#library(ggplot2)
#c5_proli <- ggplot(mki67_c5median, aes(x = c5_median, y = MKI67)) +
#  geom_point(inherit.aes = TRUE,color = "grey40", size = 1.5) +
#  xlim(-1,1) +
#  geom_smooth(method = "lm", color = "blue", fill = "lightblue") +
#  theme_minimal(base_size = 15)
...
#  fill = c("#e9c46a")
#)
#fit <- lm(MKI67 ~ c5_median, data = mki67_c5median)
#summary(fit)
#cor(mki67_c5median$MKI67, mki67_c5median$c5_median)

##Boxplot of proliferation score##
#xc3 <- mki67_median$c3_median
#yc5 <- mki67_median$c5_median
#gmix_i <- which(xc3 > 0 & yc5 > 0)
#gppp_i <- which(xc3 < 0 & yc5 > 0)
#gquis_i <- which(xc3 < 0 & yc5 < 0)
#goxp_i <- which(xc3 > 0 & yc5 < 0)
#met_group <- c()
#met_group[gmix_i] <- c("Mixed")
#met_group[goxp_i] <- c("OXP_leaning")
#met_group[gppp_i] <- c("PPP_leaning")
#met_group[gquis_i]<- c("Quiscent")
#mki67_median$met_group <- met_group
#mki67_median <- mki67_median[,-5]
#ggboxplot(mki67_median, x = "met_group", y = "MKI67", fill = "met_group") +
  #scale_fill_manual(values = c("Mixed" = "pink", 
   #                            "OXP_leaning" = "orange", 
    ##                          "Quiscent" = "green")) +
  #annotate("text", x = 1.1, y = 2,
   #        label = paste0("Kruskal-Wallis, p = ", signif(kruskal.test(MKI67 ~ met_group, data = mki67_median)$p.value, 3)),
    #       hjust = 0, size = 5) +
...
#########################################################Reviewer Comment###############################
####to be run before executing line 596##
sel_genes <- c("AKT1","ALDH1A1","AXL","CDH1","CDK14","CTHRC1","DAB2","DDR2","EPCAM","FZD7","LOXL2","NOTCH1",
               "POSTN","PRRX1","S100A4","SNAI1","VIM","ZEB1")
resM_sel <- resM_df[sel_genes,]
library(EnhancedVolcano)
EnhancedVolcano(resM_sel,
                lab = rownames(resM_sel),
                x = 'log2FoldChange',
                y = 'pvalue')
write.csv(resM_sel, file = "rev_comment_emt_sel_gene_volcano.csv")
setwd("/media/cuser17/DATA/Pan_Cancer/Gene_list_misc")
sel_genes <- readLines("EMTome.txt")
resM_sel <- resM_df[sel_genes,]
library(EnhancedVolcano)
EnhancedVolcano(resM_sel,
                lab = rownames(resM_sel),
                ...
                pointSize = 1,
                labSize = 3)
write.csv(resM_sel, file = "rev_comment_emtome_volcano.csv")
setwd("/home/cuser17/Documents/TCGA_HNSC_476/MS_work/consensus_clustering/Genelist_090625")

emt_tpm <- dat1[c("VIM","CDH1"),]
df <- data.frame(
  "VIM" = t(emt_tpm[1,]),
  "CDH1" = t(emt_tpm[2,]),
  "annot" = met_order_p_index$Mixed
)
library(ggplot2)
library(ggpubr)

ggplot(df, aes(x = annot, y = VIM, fill = annot)) +
  geom_boxplot(outlier.shape = NA, width = 0.6) +
  geom_jitter(width = 0.15, size = 2, alpha = 0.7) +
  stat_compare_means(
    comparisons = list(c("Mixed", "Others")),
    method = "wilcox.test",
    label = "p.format",
    tip.length = 0.02
  ) +
  theme_bw(base_family = "Times New Roman") +
  labs(x = "Category", y = "VIM tpm") +
  theme(
    legend.position = "none",
    text = element_text(family = "Times New Roman")
  )

ggplot(df, aes(x = annot, y = CDH1, fill = annot)) +
  geom_boxplot(outlier.shape = NA, width = 0.6) +
  geom_jitter(width = 0.15, size = 2, alpha = 0.7) +
  stat_compare_means(
    comparisons = list(c("Mixed", "Others")),
  ...
  theme_bw(base_family = "Times New Roman") +
  labs(x = "Category", y = "CDH1 tpm") +
  theme(
    legend.position = "none",
    text = element_text(family = "Times New Roman")
  )

sel_genes <- c("DAB2", "ZEB1", "SNAI1", "LOXL2", "VIM", "SNAI2", "CDH1", "CLDN1", "NOTCH1")
emt_9_genes <- dat1[sel_genes,]
emt9_scaled <- t(apply(emt_9_genes, 1, scale))
colnames(emt9_scaled) <- colnames(emt_9_genes)
met_order_mixed_annot <- data.frame(met_order_p_index[,1])
rownames(met_order_mixed_annot) <- rownames(met_order_p_index)
colnames(met_order_mixed_annot) <- c("Metabolic Group")
met_order_mixed_annot <- sort_by(met_order_mixed_annot, met_order_mixed_annot$`Metabolic Group`)
emt9_scaled <- emt9_scaled[,rownames(met_order_mixed_annot)]
pheatmap(emt9_scaled, color = colorRampPalette(c("#d7f0f9","#76bce8", "#1c3f9a", "#000000", "#7a0e0e", "#e04b3a", "#fca283"))(81),
         breaks = cbreak, cluster_rows = F, fontsize_row = 10.5, fontsize_col = 1.5, fontfamily = "Times",
         fontsize = 10, cluster_cols = F, annotation_col = met_order_mixed_annot, gaps_col = 434)
